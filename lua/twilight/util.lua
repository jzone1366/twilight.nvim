local hsluv = require("twilight.hsluv")
...
function util.warn(...)
	for _, msg in ipairs({ ... }) do
		vim.cmd('echohl WarningMsg | echom "Twilight: ' .. msg .. '" | echohl NONE')
	end
end
...
function util.terminal(theme)
	...
	if vim.fn.has("nvim") == 0 then
		local colors = {
			...
			theme.colors.cyan,
			theme.colors.white,
			theme.colors.black_br,
			theme.colors.red_br,
			theme.colors.green_br,
			theme.colors.yellow_br,
			theme.colors.blue_br,
			theme.colors.magenta_br,
			theme.colors.cyan_br,
			theme.colors.white_br,
		}
